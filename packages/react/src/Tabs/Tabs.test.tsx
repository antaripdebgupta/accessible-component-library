...
describe('Tabs — nested tabs', () => {
  test('inner tabs operate independently of outer tabs', async () => {
    ...
    const [outerTablist, innerTablist] = screen.getAllByRole('tablist') as [
      HTMLElement,
      HTMLElement,
    ];
    expect(within(outerTablist).getByRole('tab', { name: 'Outer 1' })).toHaveAttribute(
      'aria-selected',
      'true',
    );
    expect(within(innerTablist).getByRole('tab', { name: 'Inner 1' })).toHaveAttribute(
      'aria-selected',
      'true',
    );

    await user.click(within(innerTablist).getByRole('tab', { name: 'Inner 2' }));
    expect(within(innerTablist).getByRole('tab', { name: 'Inner 2' })).toHaveAttribute(
      'aria-selected',
      'true',
    );
    expect(within(outerTablist).getByRole('tab', { name: 'Outer 1' })).toHaveAttribute(
